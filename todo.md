...
## Car Mode — Aggressive Pre-Buffering (Jul 2026)
- [x] Add carMode state + toggleCarMode to AudioPlayerContext with localStorage persistence
- [x] Implement 30-second pre-buffer gate in loadAndPlay — holds playback until 30s buffered before starting
...
## Platinum Feature: Add Vocals Workflow (Jul 2026)
- [x] Read StemSplit client, mixer module, and routers to map existing infrastructure
- [x] Add vocalProjects table to schema (fusionTrackUrl, vocalStemSourceUrl, lyrics, styleAnchor, status, resultUrl, userId, timestamps)
- [x] Add tRPC procedures: start, get, list, getVocalStemOptions (vocalProjects router)
- [x] Server pipeline: MiniMax Quick Generate → StemSplit vocal extraction → ffmpeg mix → S3 store (server/vocalPipeline.ts)
- [x] Add Vocals UI panel (AddVocalsPanel.tsx): 4-step workflow, pipeline progress, past projects list, play/download
- [x] Gate behind Platinum subscription check (isPremium)
