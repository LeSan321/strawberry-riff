...
export async function createMusicGeneration(
  data: Omit<MusicGeneration, "id" | "createdAt" | "updatedAt" | "completedAt" | "structureFingerprint" | "accentProfileId"> & { structureFingerprint?: string | null; accentProfileId?: string | null }
): Promise<number | null> {
  const db = await getDb();
  if (!db) return null;
